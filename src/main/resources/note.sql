...
`user_icon` varchar(64) NOT NULL comment '用户头像',
...
`category_description` varchar(64) Not NULL comment '类目描述',
`create_time` timestamp NOT NULL default current_timestamp comment '创建时间',
`update_time` timestamp NOT NULL default current_timestamp on update current_timestamp comment '更新时间',
PRIMARY KEY (`category_id`),
key `category_name` (`category_name`),
constraint `note_category_fk_1` foreign key (`user_id`) references `user`(`user_id`)
...
`note_name` varchar(64) Not NULL comment '笔记名称',
`note_label` varchar(64) Not NULL comment '笔记标签',
`note_description` varchar(64) Not NULL comment '笔记描述',
...
`create_time` timestamp NOT NULL default current_timestamp comment '创建时间',
`update_time` timestamp NOT NULL default current_timestamp on update current_timestamp comment '更新时间',
constraint `note_info_fk_1` foreign key (`user_id`) references `user`(`user_id`),
constraint `note_info_fk_2` foreign key (`category_id`) references `note_category`(`category_id`),
